#ifndef OOK_TEST_UTILITIES_H_
#define OOK_TEST_UTILITIES_H_
...
generate_binary_matrix(std::mt19937& rng, const int n, const int m)
{
    ...
    auto idx_gen =
        std::bind(std::uniform_int_distribution<>(0.0, m - 1), std::ref(rng));
                ...
                const double sd = 1)
{
    Vector v(n);
    auto norm_gen =
        std::bind(std::normal_distribution<>(mean, sd), std::ref(rng));
