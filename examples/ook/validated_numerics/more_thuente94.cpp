...
struct algo_result
{
    ...
    {
        out << setw(6) << r.nprob << setw(6) << r.ntry << setw(6) << r.nfev
            << setw(6) << (r.info == ook::message::convergence) << setw(11)
            << scientific << r.stp0 << setw(10) << scientific << r.stp
            << setw(10) << scientific << r.f << setw(10) << scientific << r.g;
        return out;
    }
};

problem probs[] = {{1, 4, 1.e-3, 1.e-03, 1.e-01},
...
main()
{
    ...
    cout
        << "\nThis program validates that the translation of the More Thuente\n"
           "line search algorithm remains faithful to the implementation "
           "described\n"
           "in the paper\n"
    ...
    for (const auto& p : probs)
    {
        ...
        for (int ntry = 0; ntry < ntries; ++ntry)
        {
            ...
            ook::line_search::options<double> opts(
                ftol, gtol, 0, 4.0 * std::max(1.0, stp));
            ook::message msg;
            double f, g;
            ...
            results.push_back(
                {msg, nfev, ntry + 1, nprob, factor * stp0, stp, f, g});
            factor *= 100.;
        }
        cout << line << "\n" << table[p.nprob - 1] << endl;
        cout << " Summary of  " << ntries << " calls to dcsrch\n"
                                             "   ftol = "
             << scientific << ftol << "   gtol = " << scientific << gtol
             << "   g0 = " << scientific << g0 << "\n\n";
        cout << setw(7) << "nprob" << setw(6) << "ntry" << setw(6) << "nfev"
