...
struct lsm_state : public SchemeState
{
    ...
    operator<<(std::ostream& out, const lsm_state& s)
    {
        ...
        {
            out << "\n"
                << std::setw(8) << "n" << std::setw(8) << "nfev"
                << std::scientific << std::setw(16) << "a" << std::setw(16)
                << "fx" << std::setw(16) << "max ||dfx||" << std::setw(16)
                << "max ||dx||";
        }

        if (s.state == lsm_state::tag::iterate)
        {
            out << std::setw(8) << s.iteration << std::setw(8) << s.nfev
                << std::scientific << std::setw(16) << s.a << std::setw(16)
                << s.fx << std::setw(16) << s.gnorm << std::setw(16) << s.xnorm;
        }

        if (s.state == lsm_state::tag::final)
        {
            out << "\nstatus : " << s.msg << "\n"
                << std::setw(8) << "iter" << std::setw(8) << "nfev"
                << std::setw(16) << "fx" << std::setw(16) << "max ||dfx||"
                << std::setw(16) << "max ||dx||\n"
                << std::setw(8) << s.iteration << std::setw(8) << s.nfev
                << std::scientific << std::setw(16) << s.fx << std::setw(16)
                << s.gnorm << std::setw(16) << s.xnorm;
        }
        return out;
    }
...
struct line_search_method
{
    ...
    operator()(F obj_fun, X x, const Options& opts, Observer& observer) const
    {
        ...
        typedef detail::call_selector<
            std::tuple_size<decltype(obj_fun(x))>::value>
            caller_type;
        ...
        while (true)
        {
            ...
            const bool u2 = state.xnorm <= dx_eps * (1.0 + norm_inf(x));
