...
void
initial_report(const lbfgs_options<T>& opts, T gnorm,
                const X& x, T f, const X& g)
{
    if (opts.iprint[0] < 0) return;
    printf(fmts::stars);
...
template <typename T, typename X>
void
report(const lbfgs_options<T>& opts, int iter, int nfun, T gnorm,
        const X& x, T f, const X& g, T stp, bool finish)
{
    auto iprint = opts.iprint;
    if (iprint[0] < 0) return;
