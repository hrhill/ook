...
{
    /// \brief Calculate the central difference approximation to the gradient of
    /// f.
    template <typename F, typename X>
    static auto
    gradient(F f, X x);

    /// \brief Calculate the central difference approximation to the hessian of
    /// f.
...
central_difference::hessian(F f, const X& x)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        H(i, i) =
            (-fpp + 16.0 * fp - 30.0 * fx + 16.0 * fm - fmm) / (12.0 * hi * hi);
